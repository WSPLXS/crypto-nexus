...
export const GPU: React.FC<GPUProps> = ({ tier, isMining }) => {
  // Ограничиваем тир от 1 до 10
  const t = Math.max(1, Math.min(10, Math.ceil(tier / 10)));
  ...
  const getConfig = (tierNum: number) => ({
    ...
    rgbIntensity: tierNum >= 3 ? (tierNum - 2) * 0.4 : 0,
          ...
          {Array.from({ length: cfg.fans }).map((_, i) => {
            ...
            return (
              <g key={`fan-${i}`} ref={i === 0 ? fanRef : undefined} className="fan-blade" style={{ transformOrigin: `${cx}px 80px` }}>
                <circle cx={cx} cy="80" r="32" fill="#000" stroke={t >= 6 ? cfg.accentColor : '#555'} strokeWidth="3" />
                ...
                <circle cx={cx} cy="80" r="8" fill={t >= 7 ? cfg.accentColor : '#444'} />
              </g>
            );
          })}

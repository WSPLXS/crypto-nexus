...
export const CasinoModal: React.FC<CasinoModalProps> = ({ 
  ...
  const handleExchange = (direction: 'buy' | 'sell') => {
    ...
    if (direction === 'buy') {
      if (bankUsd < num) return alert('Недостаточно средств на счете');
      onChipExchange(chips + num, bankUsd - num, bankRub);
    } else {
  ...
  const playGame = (game: any) => {
    ...
    setTimeout(() => {
      ...
      if (win) {
        onChipExchange(chips + bet, bankUsd, bankRub);
      ...
      <div style={styles.modal} onClick={e => e.stopPropagation()}>
        ...
        {tab === 'exchange' && (
          <div style={styles.content}>
            ...
            <div style={styles.balanceInfo}>На счету: ${bankUsd.toFixed(2)}</div>
            <input 
              style={styles.input} 
              placeholder="Сумма" 
              type="number" 
...
const styles: any = {
  ...
  balanceInfo: { color: '#737373', fontSize: 13, marginBottom: 8 },

...
import { Handshake, MessageCircle, Crown, Pencil, Check, X } from 'lucide-react';
...
function App() {
  ...
  const loadSocial = async () => {
    await Promise.all([fetchClanData(), fetchFriendsData()]);
  };
            ...
            <div style={styles.modal} onClick={e => e.stopPropagation()}>
              ...
              {!myClan ? (
                <>
                  ...
                  <button onClick={() => alert('Поиск кланов')} style={styles.btnSecondary}>Найти клан</button>
                </>
              ) : (
                <>
        ...
        {showCreateClan && (
          <div style={styles.overlay} onClick={() => setShowCreateClan(false)}>
            <div style={styles.modal} onClick={e => e.stopPropagation()}>
              ...
              <input placeholder="Название (до 25)" maxLength={25} style={styles.input} />
              <textarea placeholder="Описание (до 200)" maxLength={200} style={{...styles.input, height: 60, resize: 'none'}} />
              <label style={styles.label}><input type="checkbox" /> Вступление по заявке</label>
              <label style={styles.label}>Мин. уровень: <input type="number" min={1} max={30} defaultValue={1} style={{width: 40, background: '#262626', border: '1px solid #404040', color: 'white', borderRadius: 4, padding: 2}} /></label>
              <label style={styles.label}>Макс. участников: <input type="number" min={5} max={1000} defaultValue={50} style={{width: 60, background: '#262626', border: '1px solid #404040', color: 'white', borderRadius: 4, padding: 2}} /></label>
              <div style={{display:'flex', gap:8, marginTop: 12}}>
                <button onClick={() => setShowCreateClan(false)} style={styles.btnSecondary}>Отменить</button>
                <button onClick={handleCreateClan} style={styles.btnPrimary}>Создать</button>
              </div>
            </div>
          </div>
        )}
...
const styles: { [key: string]: React.CSSProperties } = {
  ...
  btnSecondary: { flex: 1, padding: '12px', borderRadius: 12, border: '1px solid rgba(156,163,175,0.2)', background: 'transparent', color: '#a3a3a3', cursor: 'pointer' },
